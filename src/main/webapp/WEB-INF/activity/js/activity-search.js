$(function () {
    ...
    if (firstKey.toString() === "city") {
    ...
    } else if (thirdKey.toString() === "city-name") {

        fetch(`${APP_CONFIG.BASE_URL}activity/FilterAllActivities?${urlParams.toString()}`)
            .then(resp => {
                if (resp.ok) {
                    return resp.json();
                } else {
        ...
        console.log(`${APP_CONFIG.BASE_URL}activity/SearchActivities?${urlParams.toString()}`);
    ...
    // 點擊畫面的搜尋按鈕
    $("#filter-activities").on("click", function (e) {
        e.preventDefault();

        let searchActivityName = $("input#search-act-name").val();
        let category = $("#category-select input[type='radio']:checked").val();
        let city = $("#city-select input[type='radio']:checked").val();

        console.log("活動名稱:" + searchActivityName + category + "," + city);

        let params = new URLSearchParams();
        params.append("search-activity-name", searchActivityName);
        params.append("category", category);
        params.append("city-name", city);
